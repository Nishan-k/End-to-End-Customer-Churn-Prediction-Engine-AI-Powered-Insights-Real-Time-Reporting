...
def explain():
    ...
    if prediction == "Churn":
        st.error(f"Customer is predicted to churn with {churn_probability:.2f}% probability")
        st.session_state.churn_prob = churn_probability
    else:
        stay_probability = 100 - churn_probability
        st.success(f"Customer is predicted to stay with {stay_probability:.2f}% probability")
        st.session_state.non_churn_prob = stay_probability
